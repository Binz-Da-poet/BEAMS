...
const STORAGE_KEY = 'jacketOrder.form.v1';
...
export default function JacketOrderPage(): React.ReactElement {
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      // also read global merged form from drafts load
  ...
  const saveDraft = React.useCallback(() => {
    ...
    try {
      ...
      localStorage.setItem(STORAGE_KEY, JSON.stringify(data));
      // use shared util (validates staffName, merges all forms, and navigates to /drafts)
      saveDraftUtil('jacketOrder', data, 'jacket', data.fabricCode);
    } catch {}
  }, []);
  ...
  return (
    <div className="container-card">
      ...
      <form ref={formRef} id="jacketOrderForm" className="grid grid-cols-[220px_1fr] gap-x-5 gap-y-4">
        {/* Persist item/plan into storage and drafts */}
        ...
        <fieldset className="contents">
          ...
          <div>
            <select className="w-full h-11 rounded-lg border px-4 text-sm bg-gray-50 focus:outline-none focus:border-slate-800 focus:bg-white focus:shadow-[0_0_0_3px_rgba(31,41,55,0.08)]" id="patternNo" name="patternNo" defaultValue="">
              ...
              <option value="JKT001">JKT001 - ジャケットパターン1</option>
              <option value="JKT002">JKT002 - ジャケットパターン2</option>
            </select>
          </div>
        </fieldset>
